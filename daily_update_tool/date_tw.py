...
file_path = r'C:\Users\eric\Desktop\aff_folder\hsiav.github.io\tw\index.md'
...
# 定义一个正则表达式来匹配日期格式
date_pattern = re.compile(r'\d{4}年\d{1,2}月\d{1,2}號')

# 检查并更新日期
for i, line in enumerate(lines):
    if line.startswith("# 翻墻軟體中國VPN推薦"):
        print(line)
        new_date = datetime.datetime.now()
        new_date = new_date +  datetime.timedelta(hours=13)
        new_date = new_date.strftime('%Y%m%d')
        new_date_encoded = new_date[:4]+'年'+new_date[4:6]+'月'+new_date[6:]+'號'
        # 使用正则表达式找到并替换日期
